...
# import math
# """
# разрез на супер маленькие, но не режет боксы
# """
# images_dir = "images"
# labels_dir = "labels"
# tiles_dir = "tiles"
# tile_size = 1024
# overlap = 0.2
# pad = 0.1  # для object-centered тайлов
#
...
#     w = bw / tile_w
#     h = bh / tile_h
#     return [cls, max(0, min(1, cx)), max(0, min(1, cy)), min(1, w), min(1, h)]
#
#
...
#
#     # 2. Object-centered тайлы
#     for b in boxes:
#         cls, l, t, r, btm, bx, by, bw, bh = yolo_to_xyxy(b, W, H)
#         # если bbox уже полностью в каком-то тайле — пропускаем
#         if any(l >= tx0 and r <= tx1 and t >= ty0 and btm <= ty1 for tx0, ty0, tx1, ty1 in created):
#             continue
#
#         # создаем вокруг бокса
#         size = min(tile_size, max(int(max(bw, bh) * (1 + pad)), 128))
